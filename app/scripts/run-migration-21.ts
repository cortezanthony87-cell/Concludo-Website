...
const sections = [
  // 1. Alter organizations table and create knowledge_nodes table
...
CREATE INDEX IF NOT EXISTS idx_lessons_learned_deleted_at ON public.lessons_learned(deleted_at);`,
  ...
  `
DO $$ BEGIN
    DROP POLICY IF EXISTS "Users can view authorized knowledge nodes" ON public.knowledge_nodes;
    DROP POLICY IF EXISTS "Users can insert knowledge nodes" ON public.knowledge_nodes;
    DROP POLICY IF EXISTS "Users can update authorized knowledge nodes" ON public.knowledge_nodes;
    DROP POLICY IF EXISTS "Users can delete authorized knowledge nodes" ON public.knowledge_nodes;

    DROP POLICY IF EXISTS "Users can view authorized knowledge relationships" ON public.knowledge_relationships;
    DROP POLICY IF EXISTS "Users can insert knowledge relationships" ON public.knowledge_relationships;
    DROP POLICY IF EXISTS "Users can update knowledge relationships" ON public.knowledge_relationships;
    DROP POLICY IF EXISTS "Users can delete knowledge relationships" ON public.knowledge_relationships;

    DROP POLICY IF EXISTS "Users can view authorized lessons learned" ON public.lessons_learned;
    DROP POLICY IF EXISTS "Users can insert lessons learned" ON public.lessons_learned;
    DROP POLICY IF EXISTS "Users can update authorized lessons learned" ON public.lessons_learned;
    DROP POLICY IF EXISTS "Users can delete authorized lessons learned" ON public.lessons_learned;
END $$;
...
$$ LANGUAGE plpgsql SECURITY DEFINER;`,
  ...
  // 6. Update purge_expired_records() and grants
...
console.log(`Executing ${sections.length} migration sections for Tasklet 21...`);

// Ensure active tab is the SQL editor
await invokeTool({
  toolName: 'browser',
  args: {
    actions: [
      {
        select_tab: {
          tabId: '55708CCE46CCBC1F26EE7B3A746202DB'
        }
      }
    ]
  }
