---
page: Free Tools (appended section, formerly the standalone "Free resources" page)
slug: /modules
menu_label: Free Tools
status: draft (not published)
location: Appended to the bottom of the Free Tools page, below the card grids and above the footer.
history: |
  The standalone "Free resources" page (slug /blank-2) was deleted on 5 Aug 2026 and its menu item
  removed, per Anthony's instruction to place all this content into Free Tools.
  On 7 Aug 2026 the long article that opened this section was moved out to its own page,
  /meeting-to-action-gap, because it was crowding Free Tools. Its copy now lives in
  10-meeting-to-action-gap.md and was moved verbatim. What remains here is everything that
  followed it.
seo_title: Free Tools | Concludo (Free Tools page keeps its own existing SEO title)
meta_description: Not set. The Free Tools page meta description is still empty sitewide, pending approval.
assets:
  presentation_pdf: https://540d2ec6-f5c7-4bfa-9db6-3704a22adc51.usrfiles.com/ugd/c3e69e_13c1f4559e784b4997fd141d9d2b1aff.pdf
notes: Australian English. No em dashes or en dashes. No statistics. No pricing or checkout. Presentation trimmed from 13 to 12 pages to drop the removed slide and its Canva placeholder contact details.
rebrand: Renamed Notewright to Concludo, 24 Aug 2026 (new entity Concludo Pty Ltd; live Wix site not yet updated)
---

### Want to go deeper?

Two books shaped a lot of the thinking here and are well worth your time: Steven Rogelberg's work on the science of meetings, and Priya Parker's writing on how to gather with purpose. Both make the same underlying case in their own way, that a meeting is only as good as what it sets in motion.

[Free reading: the meeting to action gap](/meeting-to-action-gap)

## Meetings, actually

A short conversation about getting real value from the room. Two colleagues talk through why good meetings still stall, and the one habit that closes the gap.

[View the presentation](https://540d2ec6-f5c7-4bfa-9db6-3704a22adc51.usrfiles.com/ugd/c3e69e_13c1f4559e784b4997fd141d9d2b1aff.pdf)

---

Concludo is an independent product. It works with meeting transcripts from any source and is not affiliated with any meeting, recording or device provider. This article is free to read and share. Pricing for paid products is not yet open and will be confirmed before anything can be purchased.
